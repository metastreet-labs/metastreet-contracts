...
export async function extractEvent(tx: ContractTransaction, address: string, contract: Contract, name: string, index: number = 0): Promise<LogDescription> {
    ...
    if (eventCandidates.length == 0)
        throw new Error(`Event "${name}" from contract ${address} not found in logs`);
...
export async function countEvent(tx: ContractTransaction, address: string, contract: Contract, name: string): Promise<number> {
    const logs = (await tx.wait()).logs;
...
export async function expectEvent(tx: ContractTransaction, address: string, contract: Contract, name: string, args: { [name: string]: any }, index: number = 0) {
